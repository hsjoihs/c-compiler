...
- enumの初期化子をコメントアウトしたり、ネストした定義の構造体を分割したりして、`header.h`を整理したので `alignment.c` をセルフホストできるようになった。
...
- ということで構造体を関数で渡している部分をポインタ渡しにするようにしたので、 `codegen_expression` をセルフコンパイルできた。
