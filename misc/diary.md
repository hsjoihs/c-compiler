...
- ところで、私の慣習によると、 コンパイラの挙動に変化が発生していないときは Day 97 といったカウントをインクリメントしなくてよいっぽいので日記から削っておいた。

# 二年弱が経過した

## 2022年8月10日 (Day 97)

- さて、あれからいろいろなことがあった。例えば、疫病が流行り続け、セキュキャンが講師とチューターだけ対面になり、私はCコンパイラ班の講師をいま府中でやっている。

- せっかくなので、昨日自分のコンパイラを読み返していて気になったところを処理していこう。

- まず、[「ローカル変数は必ず 4 バイト以上確保する」という実装](https://github.com/hsjoihs/c-compiler/blob/c4dfc46ac9be116e6cbb8dd36f04ba55dd35a290/parse_analyze_statement.c#L22-L23)がなんかある。これはなんだろう。
...
- 「常に 4 バイトで書き込んでいて、それによる破壊をごまかすため」か？それはあり得ない。char 配列に書き込むテストケースも存在しているはずだからだ。じゃあこの条件分岐は不要なのでは？

- ということで、削ってみたら普通にテストケース全部通った。なるほどなぁ。
...
- なるほど、`parse_dcl_postfixes()` で作った `TypeNode` 配列を、`from_type3_to_type()` という関数で正当な型へと変換してるのね。じゃあ検閲はここでやる必要がある。とりあえず関数名を `from_typenodes_to_type()` にしようかな。というかこの `TypeNode` は `TypeFragment` とかにすべきだろ。まあリファクタリングとデバッグは別コミットにすべきなので、リファクタリングは後回しとしよう。
...
- 受講生が 2048 だったり [uint256_t 作のライフゲーム](https://gist.github.com/maekawatoshiki/8ea2f392e22698593d7ee8522aabbbd0)だったりを動かしたりしている。

- さて、これらのカラフルな表示は ANSI エスケープコード（U+001B で始まるシーケンス）を標準出力に出すことで実現されている。`\e` は GNU C 拡張という扱いらしい。なんでも、「C89 策定時に盛り込もうかと思ったら、EBCDIC にはそんな文字がない」という理由で却下になったんだそうな。なるほどなぁ。
...
- `\e` は非 `-pedantic` なときのみ処理するようにし、`-pedantic` だとエラーにするようにした
...
* INTEGER_CLASSを関数に渡すところは対応できていない

* 引数の数が7個以上の奇数である場合に対応できていない（どうせスタックのアラインでしょうな）

* 複数のトークンに展開される `#define` ができていない

* というかプリプロセッサが全体的にテキトー
...
- [ドーナツがまわるアニメーション](https://www.a1k0n.net/2011/07/20/donut-math.html) を教えてもらった。しかし私のコンパイラにも受講生のコンパイラにも浮動小数点数がない。

- 「こういうところで頑張るのが物理学科だろう」という意気込みで、なんとか浮動小数点数を撲滅したバージョンを作成。
