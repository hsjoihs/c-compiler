...
1stgen:
	gcc -Wall -Wextra -DOVERRIDE_STD -g std.c codegen.c alignment.c parse_analyze_toplevel.c parse_analyze_statement.c codegen_expression.c main.c vector.c typecheck_expression.c parse_expression.c error.c type.c parse_type.c map.c print_x86_64.c $(OSFLAG) lexer.c main2.c codegen_switch.c -o out/compiler.out
...
2ndgen:
	...
	gcc -Wall -Wextra -DOVERRIDE_STD self_compile_asm/std.s self_compile_asm/codegen.s self_compile_asm/alignment.s parse_analyze_toplevel.c parse_analyze_statement.c self_compile_asm/codegen_expression.s self_compile_asm/main.s main2.c self_compile_asm/vector.s typecheck_expression.c parse_expression.c self_compile_asm/error.s type.c parse_type.c self_compile_asm/map.s self_compile_asm/print_x86_64.s codegen_switch.c $(OSFLAG) lexer.c -o out/compiler.out
